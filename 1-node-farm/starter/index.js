...
const { replaceTemplate, PI } = require('./modules/replaceTemplate');
...
// dataObj = dataObj.map((el) => ({ ...el, slug: slugify(el.productName, { lower: true }) }));
dataObj.forEach((el) => {
  const slug = slugify(el.productName, { lower: true });
  el.slug = slug;
});
